...
class DetectionNetwork(object):
    ...
    def build_loss(self, rpn_box_pred, rpn_bbox_targets, rpn_cls_score, rpn_labels,
        ...
        return loss_dict
    def add_heatmap(self, feature_maps, name, vmax):

        def figure_attention(activation):
            fig, ax = tfp.subplots()
            im = ax.imshow(activation, cmap='jet', vmax=vmax)
            fig.colorbar(im)
            return fig
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch):
        ...
        with tf.variable_scope('build_rpn',
            ...
            for level_name, p in zip(cfgs.LEVLES, P_list):
                if cfgs.SHARE_HEADS:  # SHARE_HEADS = True
                ...
                rpn_conv3x3 = slim.conv2d(
                    ...
                    scope=scope_list[0],
                    reuse=reuse_flag)
                pyramid_dict_p[level_name] = rpn_conv3x3
            P_list_new = [pyramid_dict_p[level_name] for level_name in cfgs.LEVLES]
            for level_name, p in zip(cfgs.LEVLES, P_list_new):
                if cfgs.SHARE_HEADS:
                     reuse_flag = None if level_name==cfgs.LEVLES[0] else True
                     scope_list=['rpn_conv/3x3', 'rpn_cls_score']
                else:
                     reuse_flag = None
                     scope_list= ['rpn_conv/3x3_%s' % level_name, 'rpn_cls_score_%s' % level_name]
                with tf.variable_scope("branch0"):
                    branch0_c = slim.conv2d(p,
                                                       num_outputs=171,
                                                       kernel_size=[1, 1], stride=1,
                                                       weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                       scope='branch0_c' + level_name)
                    branch0_d = slim.conv2d(branch0_c,
                                                        num_outputs=171,
                                                        kernel_size=[3, 3], stride=1, rate=1, activation_fn=None,
                                                        weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                        scope='branch0_d' + level_name)
                    branch0_output = slim.batch_norm(branch0_d, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                            updates_collections=tf.GraphKeys.UPDATE_OPS, scope='scope1'+level_name)
                with tf.variable_scope("branch1"):
                    branch1_reduce_dim = slim.conv2d(p,
                                                       num_outputs=171,
                                                       kernel_size=[1, 1], stride=1,
                                                       weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                       scope='branch1_reduce_dim' + level_name)
                    branch1_c33 = slim.conv2d(branch1_reduce_dim,
                                                         num_outputs=171,
                                                         kernel_size=[3, 3], stride=1,
                                                         weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                         scope='branch1_c33' + level_name)
                    branch1_c33 = slim.batch_norm(branch1_c33, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                             updates_collections=tf.GraphKeys.UPDATE_OPS, scope='scope2'+ level_name)
                    branch1_c13 = slim.conv2d(branch1_reduce_dim,
                                                         num_outputs=171,
                                                         kernel_size=[1, 3], stride=1,
                                                         weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                         scope='branch1_c13' + level_name)
                    branch1_c13 = slim.batch_norm(branch1_c13, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                             updates_collections=tf.GraphKeys.UPDATE_OPS, scope='scope3' + level_name )
                    branch1_c31 = slim.conv2d(branch1_reduce_dim,
                                                         num_outputs=171,
                                                         kernel_size=[3, 1], stride=1,
                                                         weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                         scope='branch1_c31' + level_name)
                    branch1_c31 = slim.batch_norm(branch1_c31, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                             updates_collections=tf.GraphKeys.UPDATE_OPS, scope='scope4' + level_name )
                    branch1_all = branch1_c33 + branch1_c13 + branch1_c31
                    branch1_all = slim.batch_norm(branch1_all, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                  updates_collections=tf.GraphKeys.UPDATE_OPS,
                                                  scope='scope5' + level_name)

                    branch1_output = slim.conv2d(branch1_all,
                                                       num_outputs=171,
                                                       kernel_size=[3, 3], stride=1, rate=3, activation_fn=None,
                                                       weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                       scope='branch1_output' + level_name)
                    branch1_output = slim.batch_norm(branch1_output, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                  updates_collections=tf.GraphKeys.UPDATE_OPS,
                                                  scope='scope6' + level_name)

                with tf.variable_scope("branch2"):
                    branch2_reduce_dim = slim.conv2d(p,
                                                       num_outputs=171,
                                                       kernel_size=[1, 1], stride=1,
                                                       weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                       scope='branch2_reduce_dim' + level_name)
                    branch2_c55 = slim.conv2d(branch2_reduce_dim,
                                                       num_outputs=171,
                                                       kernel_size=[5, 5], stride=1,
                                                       weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                       scope='branch2_c55' + level_name)
                    branch2_c55 = slim.batch_norm(branch2_c55, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                           updates_collections=tf.GraphKeys.UPDATE_OPS,scope='scope7'+ level_name)
                    branch2_c15 = slim.conv2d(branch2_reduce_dim,
                                                        num_outputs=171,
                                                        kernel_size=[1, 5], stride=1,
                                                        weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                        scope='branch2_c15' + level_name)
                    branch2_c15 = slim.batch_norm(branch2_c15, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                            updates_collections=tf.GraphKeys.UPDATE_OPS, scope='scope8' + level_name )
                    branch2_c51 = slim.conv2d(branch2_reduce_dim,
                                                        num_outputs=171,
                                                        kernel_size=[5, 1], stride=1,
                                                        weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                        scope='branch2_c51' + level_name)
                    branch2_c51 = slim.batch_norm(branch2_c51, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                            updates_collections=tf.GraphKeys.UPDATE_OPS, scope='scope9' + level_name )
                    branch2_all = branch2_c55+branch2_c15 + branch2_c51
                    branch2_all = slim.batch_norm(branch2_all, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                  updates_collections=tf.GraphKeys.UPDATE_OPS,
                                                  scope='scope10' + level_name)
                    branch2_output = slim.conv2d(branch2_all,
                                                       num_outputs=171,
                                                       kernel_size=[3, 3], stride=1, rate=5, activation_fn=None,
                                                       weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                                       scope='branch2_all' + level_name)
                    branch2_output = slim.batch_norm(branch2_output, decay=0.9, center=True, scale=True, epsilon=1e-5,
                                                  updates_collections=tf.GraphKeys.UPDATE_OPS,
                                                  scope='scope11' + level_name)
                branch_concat = tf.concat(
                    (branch0_output, branch1_output, branch2_output), 3)
                branch_all_dim = slim.conv2d(branch_concat,
                                    num_outputs=512,
                                    kernel_size=[1, 1], stride=1, activation_fn=None,
                                    weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                    scope='branch_all_dim' + level_name)
                # shortcut
                short = slim.conv2d(p,
                                    num_outputs=512,
                                    kernel_size=[1, 1], stride=1, activation_fn=None,
                                    weights_initializer=tf.contrib.layers.variance_scaling_initializer(),
                                    scope='short' + level_name)

                RFAM_out = branch_all_dim + short
                RFAM_out = tf.nn.relu(RFAM_out)

                rpn_cls_score = slim.conv2d(RFAM_out, self.num_anchors_per_location * 2, [1, 1], stride=1,
            ...
            pyramid_dict_PAM = {}
            pyramid_dict_PAM['N2'] = pyramid_dict_p['P2']
            temp_N3 = slim.conv2d(pyramid_dict_p['P2'],
                        num_outputs=512,
                        kernel_size=[3, 3],
                        stride=2) + pyramid_dict_p['P3']
            N3 = slim.conv2d(temp_N3,
                     num_outputs=512,
                     kernel_size=[3, 3],padding="SAME",
                     stride=1, scope='build_N3')
            pyramid_dict_PAM['N3'] = N3
            #对于p4
            #先取出p3,对P3做3*3
            temp_N4 = slim.conv2d(N3,
                                num_outputs=512,
                                kernel_size=[3, 3],
                                stride=2) + pyramid_dict_p['P4']
            N4 = slim.conv2d(temp_N4,
                             num_outputs=512,
                             kernel_size=[3, 3], padding="SAME",
                             stride=1, scope='build_N4')
            pyramid_dict_PAM['N4'] = N4
            temp_N5 = slim.conv2d(N4,
                                 num_outputs=512,
                                 kernel_size=[3, 3],
                                 stride=2) +  pyramid_dict_p['P5']
            N5 = slim.conv2d(temp_N5,
                             num_outputs=512,
                             kernel_size=[3, 3], padding="SAME",
                             stride=1, scope='build_N5')
            pyramid_dict_PAM['N5'] = N5
            N6 = slim.max_pool2d(N5, kernel_size=[1, 1], stride=2, scope='build_N6')
            pyramid_dict_PAM['N6'] = N6
            PAM_list = [pyramid_dict_PAM[level_name] for level_name in cfgs.LEVLES_PAM]
            for level_name, p in zip(cfgs.LEVLES_PAM, PAM_list):
                if cfgs.SHARE_HEADS:
                    reuse_flag = None if level_name==cfgs.LEVLES[0] else True
                    scope_list=['rpn_conv/3x3','rpn_cls_score', 'rpn_bbox_pred']
                else:
                    reuse_flag = None
                    scope_list= ['rpn_conv/3x3_%s' % level_name,  'rpn_bbox_pred_%s' % level_name]

                rpn_box_pred = slim.conv2d(p, self.num_anchors_per_location * 4, [1, 1], stride=1,
