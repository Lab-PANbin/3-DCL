...
def add_heatmap2(feature_maps, name, vmax, vmin):
    '''
    ...
    def figure_attention(activation):
        fig, ax = tfp.subplots()
        im = ax.imshow(activation, cmap='jet', vmax=vmax, vmin=vmin)
        fig.colorbar(im)
        return fig
...
def resnet_base(img_batch, scope_name, is_training=True):
    ...
    for level in range(5, 1, -1):
        ...
        add_heatmap2(pyramid_dict['P%d' % level], name='xcmLayer%d/P%d_heat' % (level, level), vmax=25, vmin=-150)
